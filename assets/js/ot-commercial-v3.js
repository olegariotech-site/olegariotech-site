(function(){
  ...
  function rewriteHero(){
    ...
    var brandline=hero.querySelector('.hero-brandline');if(brandline)brandline.textContent='Tecnologia com chão de venda';
    var sub=hero.querySelector('.hero-sub');if(sub)sub.textContent='Estruturas digitais com visão comercial para transformar presença em confiança, contatos e oportunidades de negócio.';
    var actions=hero.querySelectorAll('.hero-actions .btn');
    if(actions[0]){actions[0].href=DIAG;actions[0].innerHTML='Analisar meu negócio '+svgArrow();}
    if(actions[1]){actions[1].classList.remove('btn-wa');actions[1].classList.add('btn-ghost');internalLink(actions[1],'#projetos','Ver projetos reais');}
    var orbit=hero.querySelector('.orbit-status');
    if(orbit)orbit.innerHTML='ORBIT · OT<br>operação · <b id="orbitClock">--:--:-- BRT</b><div class="otv3-orbit-stack"><span>ecossistema conectado</span><b>Marca · Site · Google</b><b>WhatsApp · Dados</b><span>base · Valinhos/SP</span></div>';
  }

  function rewriteNavigation(){
    var railLabel=document.querySelector('.rail-label');if(railLabel)railLabel.innerHTML='O que está travando<br>seu negócio?';
    var railNames={presenca:'Parecer profissional',oferta:'Vender uma oferta',digital:'Conectar meu digital'};
    document.querySelectorAll('.desktop-rail [data-solution]').forEach(function(btn){var label=btn.querySelector('span:last-child');if(label&&railNames[btn.dataset.solution])label.textContent=railNames[btn.dataset.solution];});
    ...
    if(railContact){
      ...
      var metas=railContact.querySelectorAll('.rail-meta span');if(metas[0])metas[0].textContent='Projetos para todo o Brasil';if(metas[1])metas[1].textContent='Sites · páginas de venda · ecossistema';
    }
    var topDiag=document.querySelector('.top-nav a[href="/diagnostico-digital/"]');if(topDiag)topDiag.textContent='Diagnóstico OT';
    var mobileCta=document.querySelector('.mobile-cta');if(mobileCta)mobileCta.textContent='Diagnóstico OT';
  }
  ...
  function rewriteChoices(){
    ...
    var cards={presenca:{title:'Não pareço profissional',desc:'Meu negócio entrega bem, mas o digital não transmite a mesma confiança.'},oferta:{title:'Minha oferta não avança',desc:'Tenho produto ou serviço, mas a página não ajuda o cliente a decidir.'},digital:{title:'Meu digital está solto',desc:'Site, WhatsApp, Google e conteúdo não trabalham na mesma direção.'}};
    document.querySelectorAll('.choice-tab[data-solution]').forEach(function(btn){var data=cards[btn.dataset.solution];if(!data)return;var strong=btn.querySelector('strong'),span=btn.querySelector('span');if(strong)strong.textContent=data.title;if(span)span.textContent=data.desc;});
    var projectChoice=document.querySelector('.choice-tab[data-scroll="projetos"]');
    if(projectChoice){var ps=projectChoice.querySelector('strong'),pd=projectChoice.querySelector('span');if(ps)ps.textContent='Quero ver prova real';if(pd)pd.textContent='Projetos publicados, decisões e estruturas que já saíram do papel.';}
  }

  var solutionCopy={
    presenca:{index:'01 · rota comercial',title:'Presença profissional',text:'Para negócios que entregam bem, mas ainda não transmitem o mesmo nível de confiança no digital.',bullets:['Apresentação clara e confiável','Contato fácil no celular','Estrutura para ser encontrado no Google','Domínio e publicação organizados','Base pronta para medir evolução'],cta:'Analisar esta rota',href:DIAG+'?rota=presenca'},
    oferta:{index:'02 · rota comercial',title:'Oferta que converte',text:'Para produto, serviço ou campanha que precisa explicar valor, responder objeções e encurtar o caminho até a ação.',bullets:['Oferta entendida rapidamente','Benefícios e objeções organizados','Caminho curto até a ação','Mensagem pronta para campanha','Medição de cliques e contatos'],cta:'Analisar esta rota',href:DIAG+'?rota=oferta'},
    digital:{index:'03 · rota comercial',title:'Ecossistema conectado',text:'Para negócios que já usam várias ferramentas, mas ainda operam com site, WhatsApp, Google e dados como peças soltas.',bullets:['Uma rota para site, WhatsApp e Google','Menos ferramentas desconectadas','Dados que mostram origem das visitas','Atendimento com mais contexto','Evolução organizada por prioridade'],cta:'Analisar esta rota',href:DIAG+'?rota=ecossistema'}
  };

  function renderCommercialSolution(key){
    var item=solutionCopy[key],content=document.getElementById('solutionContent');if(!item||!content)return;
    ...
    var meta=document.querySelector('#solutionMedia .solution-meta');if(meta)meta.textContent='mobile · Google · WhatsApp · dados';
    var selection=document.getElementById('heroSelection');if(selection)selection.innerHTML='Rota selecionada · <b>'+item.title+'</b>';
  }

  function installSolutionLayer(){
    var active=document.querySelector('.choice-tab[data-solution].is-active');renderCommercialSolution(active?active.dataset.solution:'presenca');
    document.querySelectorAll('[data-solution]').forEach(function(btn){btn.addEventListener('click',function(){var key=btn.dataset.solution;setTimeout(function(){renderCommercialSolution(key);},0);});});
  }
  ...
  function enhanceCurrentProject(){
    ...
    var active=document.querySelector('.project-tab.is-active'),key=active&&active.dataset.project,data=projectDecision[key];if(!data)return;
    var copy=stage.querySelector('.project-copy');if(!copy)return;
    var old=copy.querySelector('.otv3-decision');if(old)old.remove();
    var block=document.createElement('div');block.className='otv3-decision';block.dataset.project=key;block.innerHTML='<b>Decisão OT</b><p>'+data.decision+'</p><span class="otv3-evidence">'+data.evidence+'</span>';
    var actions=copy.querySelector('.project-actions');if(actions)copy.insertBefore(block,actions);else copy.appendChild(block);
    if(actions){var secondary=actions.querySelector('.btn-ghost');if(secondary)internalLink(secondary,DIAG+'?origem=case&projeto='+encodeURIComponent(key),'Analisar meu negócio');}
  }
  ...
  function rewriteMethod(){
    ...
    var steps=[['01','Diagnosticar','Entender negócio, cliente, oferta, objeções e prioridade antes de recomendar a solução.'],['02','Estruturar','Definir mensagem, jornada, conteúdo e o caminho mais curto até a ação certa.'],['03','Construir','Transformar a estratégia em design, código, conteúdo e experiência responsiva.'],['04','Conectar','Organizar WhatsApp, Google, mensuração e canais para trabalharem na mesma direção.'],['05','Evoluir','Medir, ajustar e ampliar a estrutura conforme o negócio ganha maturidade e novas necessidades.']];
    var grid=section.querySelector('.method-grid');if(grid)grid.innerHTML=steps.map(function(m){return '<article class="method-card"><b>'+m[0]+'</b><strong>'+m[1]+'</strong><p>'+m[2]+'</p></article>';}).join('');
    var acc=document.getElementById('methodAccordion');if(acc)acc.innerHTML=steps.map(function(m,i){return '<article class="method-item'+(i===0?' is-open':'')+'"><button class="method-toggle" type="button" aria-expanded="'+(i===0?'true':'false')+'"><span><b>'+m[0]+'</b>'+m[1]+'</span><i>+</i></button><div class="method-panel"><div><p>'+m[2]+'</p></div></div></article>';}).join('');
  }
  ...
  function rewriteAbout(){
    ...
    var stats=section.querySelector('.about-stats');if(stats)stats.innerHTML='<span>30+ anos de experiência comercial do fundador</span><span>Base em Valinhos/SP</span><span>Projetos para todo o Brasil</span>';
  }
  ...
  function insertInvestmentAndFaq(){
    ...
    var trust=document.getElementById('seguranca'),products=document.getElementById('produtos');if(!trust||!products)return;
  ...
  function rewriteFinalCta(){
    ...
    var actions=cta.querySelectorAll('.hero-actions .btn');if(actions[0]){actions[0].href=DIAG;actions[0].textContent='Quero meu Diagnóstico OT';}if(actions[1])actions[1].textContent='Falar direto com a OT';
  }
  ...
  function rewriteFooter(){
    ...
    var metas=footer.querySelectorAll('.footer-meta span');if(metas[0])metas[0].textContent='Base em Valinhos/SP · projetos para todo o Brasil';
  ...
  function setupPlanetInteraction(){
    var hero=document.getElementById('inicio'),wrap=document.getElementById('earthOrbit');if(!hero||!wrap||matchMedia('(max-width:900px)').matches||matchMedia('(prefers-reduced-motion:reduce)').matches)return;
    hero.addEventListener('pointermove',function(e){var r=hero.getBoundingClientRect(),nx=(e.clientX-r.left)/r.width-.5,ny=(e.clientY-r.top)/r.height-.5;wrap.style.setProperty('--ot-planet-x',(nx*5).toFixed(1)+'px');wrap.style.setProperty('--ot-planet-y',(ny*4).toFixed(1)+'px');wrap.style.setProperty('--ot-planet-rx',(-ny*2.5).toFixed(2)+'deg');wrap.style.setProperty('--ot-planet-ry',(nx*3.2).toFixed(2)+'deg');},{passive:true});
  ...
  function initDiagnostic(){
    ...
    if(lead){
      ...
      if(!card.querySelector('.otv3-diagnostic-outcomes')){var outcomes=document.createElement('div');outcomes.className='otv3-diagnostic-outcomes';outcomes.innerHTML='<span><b>Até 3 pontos</b>clareza, confiança e contato</span><span><b>1 prioridade</b>o que atacar primeiro</span><span><b>1 rota</b>presença, conversão ou ecossistema</span>';lead.after(outcomes);}
    }
    var proofs=card.querySelectorAll('.proof');if(proofs[0])proofs[0].innerHTML='<b>4 perguntas</b><span>Rápido e direto.</span>';if(proofs[1])proofs[1].innerHTML='<b>Gratuito</b><span>Sem obrigação de contratar.</span>';if(proofs[2])proofs[2].innerHTML='<b>Com contexto</b><span>A conversa já começa do ponto certo.</span>';
    ...
    if(resultStep){var rtitle=resultStep.querySelector('.question');if(rtitle)rtitle.textContent='Seu mapa inicial está pronto.';var rhint=resultStep.querySelector('.hint');if(rhint)rhint.textContent='Esta leitura usa suas respostas para apontar a primeira direção. A análise completa ganha contexto na conversa com a OT.';var wa=resultStep.querySelector('.wa-big');if(wa)wa.textContent='Levar este diagnóstico para a OT →';var small=resultStep.querySelector('.small');if(small)small.textContent='Leitura inicial, sem promessa de resultado. Vendas também dependem de oferta, tráfego, preço, atendimento e operação.';}
    var finish=card.querySelector('[data-finish]');if(finish)finish.addEventListener('click',function(){setTimeout(buildDiagnosticMap,30);});
  }

  function selectedValue(name){var el=document.querySelector('.opts[data-name="'+name+'"] .opt.selected');return el?(el.dataset.value||el.textContent.trim()):'';}

  function buildDiagnosticMap(){
    var estrutura=selectedValue('estrutura'),objetivo=selectedValue('objetivo'),negocio=selectedValue('negocio'),points=[];
    ...
    var priority='Organizar uma presença profissional antes de adicionar novas ferramentas.',route='Presença profissional';
    ...
    if(summary)summary.innerHTML='<strong>Mapa inicial OT</strong><div class="otv3-result-map">'+points.slice(0,3).map(function(p,i){return '<div class="otv3-result-point"><b>Ponto '+(i+1)+'</b><span>'+p+'</span></div>';}).join('')+'</div><div class="otv3-result-priority"><strong>Prioridade:</strong> '+priority+'<br><strong>Rota sugerida:</strong> '+route+'</div>';
    var wa=document.getElementById('waLink');
    if(wa){var extra='\n\nLeitura inicial OT:\n- '+points.slice(0,3).join('\n- ')+'\nPrioridade: '+priority+'\nRota sugerida: '+route;if(wa.href.indexOf('Leitura%20inicial%20OT')===-1)wa.href+=encodeURIComponent(extra);}
  }

  ready(function(){initHome();initDiagnostic();});
})();
